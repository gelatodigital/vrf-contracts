...
import {ChainOptions, fetchBeacon, HttpCachingChain, HttpChainClient} from 'drand-client'
...
describe("VRF Test Suite", function () {
  ...
  before(async function () {
    ...
    // Drand testing client
    chain = new HttpCachingChain(`https://api.drand.sh/${CHAIN_HASH}`, DRAND_OPTIONS)
  ...
  it("Stores the latest round in the mock consumer", async () => {
    ...
    const { round: receivedRound, randomness } = await fetchBeacon(client, requestedRound);

    const latestRound = await mockConsumer.latestRound()
    expect(await mockConsumer.beaconOf(latestRound)).to.equal(ethers.BigNumber.from(`0x${randomness}`))
